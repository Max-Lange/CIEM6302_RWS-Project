...
st.write('In this first section, the results of the algorithm are shown for 80, 100, and 120 inspectors. It is important to notice that the graphs displayed are images, as the running time for a desired inspector number is around 6 minutes.')

# Widget for inspector number

selected_insp_number = st.selectbox('Select Inspector Number:', ['80', '100', '120'])

inspectors80 = Image.open('./Images/80inspectors.png')
inspectors100 = Image.open('./Images/100inspectors.png')
inspectors120 = Image.open('./Images/120inspectors.png')

def show_insp_img (selected_insp_number):
    if selected_insp_number == '80':
        st.image(inspectors80, caption=selected_insp_number + ' Inspectors', use_column_width=True)
    if selected_insp_number == '100':
        st.image(inspectors100, caption=selected_insp_number + ' Inspectors', use_column_width=True)
    if selected_insp_number == '120':
        st.image(inspectors120, caption=selected_insp_number + ' Inspectors', use_column_width=True)

show_insp_img(selected_insp_number)
...
def load_data():
    ...
    return df_incident, NL_map

df_incident, NL_map = load_data()
...
# Import preloaded points_gdf 
points_gdf = gpd.read_file('./Data/Preloaded/points_gdf_preloaded.shp', crs="EPSG:28992")
#rename probabilit column to probability
points_gdf.rename(columns={'probabilit': 'probability'}, inplace=True)
...
st.write("Now, in the following widget you can select the desired number of inspectors and get the correspondign optimal placement. The algorithm will consider the entire dataset, for evaluating the algorithm with data filtering please refer to the Algorithm - Variable Data window.")
...
NL_map.to_crs("EPSG:28992", inplace=True)
